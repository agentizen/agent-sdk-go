...
// buildDecompositionSchema builds a JSON Schema for the orchestrator's decomposition output.
// The schema requires a JSON object where every key is an agent name from the roster and
// every value is a string describing the sub-task assigned to that agent.
func buildDecompositionSchema(slots []AgentSlot) map[string]interface{} {
	properties := make(map[string]interface{}, len(slots))
	required := make([]string, 0, len(slots))
	for _, slot := range slots {
		properties[slot.Agent.Name] = map[string]interface{}{
			"type":        "string",
			"description": fmt.Sprintf("Sub-task assignment for agent %s", slot.Agent.Name),
		}
		required = append(required, slot.Agent.Name)
	}
...
// how to decompose a user prompt into sub-tasks (DECOMPOSITION MODE) and synthesize
...
Analyze the user prompt and assign a focused sub-task to each agent.
...
Read all the sub-results provided and synthesize a single, coherent, comprehensive final answer
that incorporates all agent findings. Write a clear, well-structured response.`,
	...
	// Set the orchestrator's model from RunConfig when available.
	// When left nil, the runner resolves via provider.GetModel("") which
	// returns the provider's configured default model.
	if runCfg != nil && runCfg.Model != nil {
		orch.WithModel(runCfg.Model)
	}
